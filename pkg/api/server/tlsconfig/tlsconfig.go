...
func LoadFromEnv(getenv func(string) string) *Config {
	return &Config{
		MinTLSVersion:    getenv("TLS_MIN_VERSION"),
		CipherSuites:     getenv("TLS_CIPHER_SUITES"),
		CurvePreferences: getenv("TLS_CURVE_PREFERENCES"),
	}
}
...
func (c *Config) ToTLSConfig() (*tls.Config, error) {
	...
	// Set minimum TLS version (if not specified, Go's default is used - currently TLS 1.2)
	if c.MinTLSVersion != "" {
	...
	// Note: When MinVersion is 0, Go uses its default (TLS 1.2 as of Go 1.18+)
...
// Uses Go's built-in tls.CipherSuites() and tls.InsecureCipherSuites()
...
func parseCipherSuites(ciphers string) ([]uint16, error) {
	...
	for _, part := range parts {
		...
		// Unknown cipher - skip and continue
		// This allows forward compatibility with new ciphers
		continue
	}
...
func parseCurvePreferences(curves string) ([]tls.CurveID, error) {
	...
	for _, part := range parts {
		...
		// Unknown curve - skip and continue
		// This allows forward compatibility with new curves
		continue
	}
